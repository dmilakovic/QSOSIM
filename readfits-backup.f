c-------------------------------------------------------------------------------
     ...
     +                   dvavoid,ra,dec,zqso,alpha,vmag,sigblur,numlls)
...
c Declare variables
      ...
      INTEGER,DIMENSION(:),allocatable :: numlls, s2n
      REAL,DIMENSION(:),allocatable :: ra,dec,zqso,alpha,vmag,
     &                                 sigblur
      character :: filename*40,ttype(20)*10, nhills,blls,zlls
      logical :: anynull
...
c Read contents of 'QSO' (ntable=3) binary table
      ntable=3
      call ftmahd(unit,ntable,hdutype,status)
      ALLOCATE(ra(nrows),dec(nrows),zqso(nrows),alpha(nrows))
      ALLOCATE(vmag(nrows),sigblur(nrows))
      ALLOCATE(numlls(nrows),s2n(nrows))
c Read column data, one row at a time, and print them out
     ...
     &       anynull,status)
            call FTGCVJ(unit,7,irow,felem,nelems,nulle,s2n(irow),
     &       anynull,status)
            call FTGCVJ(unit,8,irow,felem,nelems,nulle,numlls(irow),
     &       anynull,status)
      end do
 200  format(i2,2x,f6.2,2x,f6.2,2x,f6.2,2x,i2)
 300  format(4x,a6,2x,a6,2x,a6,2x,a7)
      write (*,300)'RA','DEC','Z','num LLS'
      do irow=1,nrows
         write (*,200)irow,ra(irow),dec(irow),zqso(irow),numlls(irow)
      end do
c Read column names that are relevant for LLS
      call FTGKNS(unit,'TTYPE',9,20,ttype,nfound,status)
      write (*,*)'found TTYPE ',nfound
      call FTGKNE(unit,'blls',1,20,blls,nfound,status)
      call FTGKNE(unit,'zlls',1,20,zlls,nfound,status)
c------------------------------------------------------------------------------
c Close fits file
