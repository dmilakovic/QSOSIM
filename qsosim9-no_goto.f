...
c ----------------------------------------------------------------------------
	SUBROUTINE QSOSIM9(zqso,alpha,vmag,wstart,wend,dw,nc,nuplim,
     ...
     :          flerr,nnflux,npoints,xs,ys,CDDF,numlin,bigA,gamma)
Cf2py intent(in) zqso,alpha,vmag,wstart,wend,dw,nc,nuplim,sigblur,s2n
Cf2py intent(in) inoise,dvavoid,npoints,xs,ys,CDDF,numlin
Cf2py intent(out) npts,lambda,flux,flerr,nnflux
	  ...
	  real*8 flerr(262144), nnflux(262144)
          real*8 alm, fik,asm
	  real*8 n12,n13p1,n14,s1,s2
	  real*4 nlin1,nlin2,nlin3
	  real*4 factor
...
c Initialise random numbers
      ...
      do i=1,npoints
	 dummy(i)=dble(i)
	 dummyy(i)=dble(alog10(real((nc**mbp1-(10**xs(i))**mbp1)
     &    /(nc)**mbp1)))
      end do
...
c         write(*,100)lambda(i),flux(i),flerr(i),nnflux(i)
	...
	call PGENV(11.5,22.5,2.0,3.0,0,1)
	...
	call PGLINE(numlin,real(dummy),real(nhi4))
      call pgend
